...
void makeFileListConfigAlt(TString filelist, TString treename)
{
	...
	while (!fh.eof()) {
		...
		if(tree->FindBranch("mc_channel_number")) {
			//if((treename.CompareTo("susy")==0)&&(filename.Contains("McAtNlo"))) 
      tree->Draw("mc_channel_number>>runnumbers_temp","mcevt_weight[0][0]"); //R17 McAtNlo
			...
			//else 
      tree->Draw("RunNumber>>runnumbers_temp");	 //R17 data and R16 data & MC
		}
