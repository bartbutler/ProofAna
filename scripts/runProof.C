...
void runProof(TString url, TString dataset, int nwrks = -1, TString treename = "")
{
    ...
    if (fcs && fcs->GetStagedPercentage() < 10) {
      cout << "runProof ERROR: There's still a problem. Tried 3 additional times to verify the dataset and it's still not staged! f = " << fcs->GetStagedPercentage() << endl;
      return;
    }
    if (treename.CompareTo("")!=0) {
		if(proof->SetDataSetTreeName(dataset,treename)) {
    ...
    //proof->GetMissingFiles();
    proof->Process(dataset.Data(),sel.Data());
    //This breaks on PROOF-Lite, fix before making default
    if(url.CompareTo("lite://"))
      PrintMissing(proof,dataset);
