...
class ObjVecObj : public MapObj {
	...
	public:
		...
		static void InsertClone(TObject* orig, TObject* clone) {
			map<TObject*, TObject*>::iterator mapiter;
			mapiter = m_clonemap.lower_bound(orig);
			if(mapiter != m_clonemap.end() && (orig==mapiter->first)) return;
			else m_clonemap.insert(mapiter,map<TObject*,TObject*>::value_type(orig,clone));
		}
...
class MomentObj : public TObject {
	public:
		MomentObj() { }
		virtual ~MomentObj() {Reset();}

		MomentObj(const MomentObj& obj) : TObject(obj), m_keyMap(obj.m_keyMap) { //deep copies
			bool topclone = !m_clonerunning;
			if(topclone) m_clonerunning = true;
			ObjVecObj::InsertClone(const_cast<MomentObj*>(&obj),this);
		...
		virtual TObject* Clone(const char*) const { //deep copies
		...
		bool Add(const MomKey& key, TObject* obj, bool weakref = false) {
			map<MomKey,MapObj*>::iterator iter;
			if(!Find(key,iter)) {
				iter = m_keyMap.insert(iter, map<MomKey,MapObj*>::value_type(key, new ObjVecObj(weakref)));
			}
			return iter->second->Add(obj);
		}
		...
		template <class T>
		T* Obj(const MomKey& key, int index = 0) { return static_cast<T*>(Find(key)->second->Obj(index)); }
		template <class T>
