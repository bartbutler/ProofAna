...
void AnalysisChain::SetTimerOutput(TDirectory* dir)
{
	if(SaveTimers()) Time()->SetHistDir(dir);
	vector<AnalysisBase*>::iterator iter = fAnalyses.begin();
	for(; iter!=fAnalyses.end(); ++iter) (*iter)->SetTimerOutput(dir);
}
...
void AnalysisChain::SlaveBegin(TTree* /*tree*/)
{
	...
	for(; iter!=fAnalyses.end(); ++iter) {
		...
		(*iter)->Time()->Start(aName_SlaveBegin_WorkerBegin);
		...
		(*iter)->Time()->Stop(aName_SlaveBegin_WorkerBegin);
...
Bool_t AnalysisChain::Process()
{
	...
	if(Debug()) cout << "AnalysisChain: DEBUG Finish CopyEvent() execution on analysis " << Name() << ", reject="<< reject <<endl;
	...
	for(; iter!=fAnalyses.end(); ) {
		...
		AnaKey aName_Process_ProcessEvent((*iter)->Name()+a_Process_ProcessEvent);
		(*iter)->Time()->Start(aName_Process_ProcessEvent);
		...
		(*iter)->Time()->Stop(aName_Process_ProcessEvent);
...
void AnalysisChain::SlaveTerminate()
{
	...
	for(; iter!=fAnalyses.end(); ++iter) {
		...
		(*iter)->Time()->Start(aName_SlaveTerminate_WorkerTerminate);
		...
		(*iter)->Time()->Stop(aName_SlaveTerminate_WorkerTerminate);
