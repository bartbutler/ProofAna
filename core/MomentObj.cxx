...
struct pt_cmp {
  ...
  bool operator ()(TObject* a, TObject* b) const {
	...
	/*
	static TClass* pcl = Particle::Class();

    if(!(a->InheritsFrom(pcl)&&b->InheritsFrom(pcl))) Abort("MomentObj: ERROR tried to sort vector of non-ParticleSortBase objects by Pt");
    */
...
class value_cmp {
	...
	public:
		...
		bool operator ()(TObject* a, TObject* b) const {
			...
			/*
			static TClass* mobjcl = MomentObj::Class();

    		if(!(a->InheritsFrom(mobjcl)&&b->InheritsFrom(mobjcl))) Abort("MomentObj: ERROR tried to sort vector of non-ParticleSortBase objects by Pt");
    		*/
...
void MomentObj::Sort(const MomKey& key, const MomKey& sortkey, bool reverse) {
	...
	if(coll->size()) {
		if((*coll->begin())->InheritsFrom(mobjcl)) {
			if(reverse) {
...
void MomentObj::SortPt(const MomKey& key, bool reverse) {
	...
	if(coll->size()) {
		if((*coll->begin())->InheritsFrom(pcl)) {
			static pt_cmp cmp;
			cmp.reverse = reverse;
			sort(coll->begin(),coll->end(),cmp);
		}
...
void MomentObj::SortPtAll(bool reverse) {
	...
	for(;coll!=m_keyMap.end();coll++) {
		...
		if(coll->second->ObjVec()->size()) {
			if((*coll->second->ObjVec()->begin())->InheritsFrom(pcl)) {
...
void MomentObj::Show() const {
	...
	for(p = reordered_map.begin(); p!=reordered_map.end(); ++p) {
		...
		TString third;
		if(!p->second) {
			TString msg("MomentObj::Show ERROR key ");
			msg.Append(p->first);
			msg.Append(" refer to NULL pointer!");
			Abort(msg);
		}
